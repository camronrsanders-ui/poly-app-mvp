...
test('Android local runner resolves an actual Android Flutter device', () => {
  ...
  assert.match(runner, /flutter run --flavor staging -d/);
  assert.match(runner, /\$DEVICE_ID/);
});
...
test('Android local runner keeps emulator routing and project guards intact', () => {
  assert.match(runner, /FIREBASE_PROJECT_ID="polycircle-staging-82204f"/);
  assert.match(
    runner,
...
test('Android bootstrap and local staging guidance preserve permanent identities', () => {
  ...
  assert.match(runner, /com\.polycircle\.app\.staging/);
  assert.match(runner, /bash tool\/dev_preflight\.sh staging/);
  assert.match(runner, /--flavor staging/);
  assert.doesNotMatch(bootstrap, /com\.mycompany\.polycircle/);
  assert.doesNotMatch(runner, /com\.mycompany\.polycircle/);
  assert.doesNotMatch(bootstrap, /com\.example\.polycircle/);
  assert.doesNotMatch(runner, /com\.example\.polycircle/);
});
