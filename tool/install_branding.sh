...
[[ -n "$SOURCE" ]] || fail "Approved logo not found. Pass its path: bash tool/install_branding.sh /path/to/$EXPECTED_NAME"
...
if [[ -d "$ROOT_DIR/android/app/src/main/res" ]]; then
  ...
  ok "Android legacy launcher mipmaps generated from approved logo"

  if find "$ROOT_DIR/android/app/src/main/res" -path '*mipmap-anydpi-v26*' -name 'ic_launcher*.xml' -print -quit | grep -q .; then
    warn "Android adaptive-icon XML exists. Legacy mipmaps are updated, but adaptive foreground/background still require visual review before Android branding is marked complete."
  fi
else
